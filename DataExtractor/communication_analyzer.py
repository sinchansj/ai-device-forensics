...
class CommunicationAnalyzer:
    ...
    def _analyze_international_activity(self, communications):
        ...
        # Check messages for welcome texts
        for msg in communications['messages']:
            content = msg.get('content', '').lower()
            if 'welcome to' in content and any(country in content for country in ['united states', 'india']):
                country = 'India' if 'india' in content else 'United States'
                international['detected'] = True
                international['countries'].add(country)
